...
class Account:
    ...
    def time_to_retire(self, amount: float) -> int:
        ...
        "*** YOUR CODE HERE ***"
        awesome_balance = self.balance
        goal = amount
        years = 0
        while awesome_balance <= goal:
            awesome_balance += awesome_balance * self.interest
            years += 1
        return years
...
def without(s: Link, i: int) -> Link:
    ...
    if s.rest == s.empty:
        return s #why did this work

    if i == 0:
        print("DEBUG: s.rest: ", s.rest)
        return s.rest
