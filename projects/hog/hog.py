...
def boar_brawl(player_score, opponent_score):
    ...
    points_scored = 3 * abs(tens_digit - ones_digit)
    return 1 if points_scored == 0 else points_scored
    # END PROBLEM 2
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def strategy(current_score, opp_score):
        return n
    return strategy
    # END PROBLEM 6
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score in range(goal):
        for opp_score in range(goal):
            if strategy(score, opp_score) != curr:
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    count = 1
    while count <= 10:
        outcome = (make_averaged(roll_dice, times_called)(count, dice))
