...
def pick(paragraphs: list[str], select, k: int) -> str:
    ...
    print(f"DEBUG: filtered is {filtered}")
    return '' if k >= len(filtered) else filtered[k]
    # END PROBLEM 1
    ...
    if typed_word in word_list:
        ...
        for word in word_list:
            ...
            if diff < min_diff:
                min_diff = diff
                corrected_word = word
        return typed_word if min_diff > limit else corrected_word
...
def fastest_words(words_and_times: dict) -> list[list[str]]:
    ...
    for j in word_indices:
        best_player = 0
        best_time = get_time(times, 0, j)
        for p in player_indices:
            t = get_time(times, p, j)
            if t < best_time:
                best_time = t
                best_player = p
        fastest[best_player].append(words[j])
    return fastest
    # END PROBLEM 10
